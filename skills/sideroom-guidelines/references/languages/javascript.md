...
Rules every JavaScript change must follow.
...
// good
function discount(user) {
  if (!user || !user.active || !user.subscribed) {
    return 0;
  }
  return 0.2;
}
...
// good
function update(validatedProfile) {
  repository.update(validatedProfile);
}
```
...
function normalizeUser(userInput) {
  return { ...userInput, email: userInput.email.trim().toLowerCase() };
}
async function createUser(userInput) {
  validateUser(userInput);
  const normalizedUser = normalizeUser(userInput);
  await db.users.insert(normalizedUser);
  await notifyWelcome(normalizedUser.email);
...
// good — the third occurrence confirms one domain concept
...
```js
// bad
function formatPrice(value, options = defaults) {
...
```js
// bad
function createInvoice(order, recurring = false, multiCurrency = false) {
...
function calculateInvoiceTotal(invoiceId) {
  const invoice = new MysqlConnection().query(
...
**Enforcement note:** rules 1–2 are mechanical and should be backed by a JavaScript formatter/linter. Rules 3–10 are structural checks. Rules 11–19 require review judgment.
